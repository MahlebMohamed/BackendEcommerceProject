    ...
    check('subcategories')
        ...
        .custom((subCategoriesIds) =>
            SubCategory.find({ _id: { $exists: true, $in: subCategoriesIds } })
                .then((resulta) => {
                    if (resulta.length < 1 || resulta.length !== subCategoriesIds.length) {
        ...
        .custom((val, { req }) =>
            SubCategory.find({ category: req.body.category })
                .then((subCategories) => {
                    const tableauxSubCategoriesIds = [];
                    subCategories.forEach(subCategory => {
                        tableauxSubCategoriesIds.push(subCategory._id.toString());
                    });

                    function checker(target, arr) {
                        return target.every((value) => arr.includes(value));
                    }

                    if (!checker(val, tableauxSubCategoriesIds)) {
                        return Promise.reject(new Error(`No Subcategory for this id Category`))
                    }
                })
        )
    ,
